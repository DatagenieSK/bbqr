...
const validEmpCodes = [
    ...
    "IRC43580", "IR0257", "ASB0197", "IRC66135", "IR0619",
    "SK"
];
...
let history = [];
try {
    history = JSON.parse(localStorage.getItem("bbQrHistory") || "[]");
...
function checkLogin() {
    ...
    if (!code) {
        showLoginError("Please enter Employee Code.");
        return;
    }
    ...
    showLoginError("Invalid Employee Code. Try again.");
    playErrorVideo();
}
...
function showLoginError(message) {
    ...
    const loginCard = loginScreen ? loginScreen.querySelector(".card") : null;
    if (loginCard) {
...
function playErrorVideo() {
    if (!errorVideo) {
        console.error("errorVideo element not found.");
        return;
    }
    ...
    const playPromise = errorVideo.play();
...
if (empCodeInput) {
    empCodeInput.addEventListener("keydown", function (event) {
        if (event.key === "Enter") {
...
function playBossSound() {
    ...
    try {
        ...
        console.log("Boss sound error:", error);
    }
}

// =====================================================
// LOVE SOUND (SWEET CHORD)
// =====================================================
function playLoveSound() {
    ...
    try {
        ...
        console.log("Love sound error:", error);
    }
}
...
function generateRandom(type) {
    let finalCode = "";
    switch (type) {
        case "BBNOW": finalCode = "BB" + randomDigits(9); break;
        case "MED": finalCode = "PBM-" + randomDigits(10); break;
        case "HED": finalCode = "PBHM-" + randomDigits(10); break;
        case "SMALL": finalCode = "PBS-" + randomDigits(10); break;
        case "LARGE": finalCode = "PBL-" + randomDigits(10); break;
        case "SLOT": finalCode = "BN" + randomDigits(2) + "-PO01"; break;
        case "GEL": 
        case "Gel": finalCode = "PCM-BLU-SM-C" + randomDigits(4); break;
        case "IN": finalCode = "GP" + randomDigits(2) + "-IN-IBG-E" + randomDigits(4); break;
        case "BL": finalCode = "GP" + randomDigits(2) + "-KL-BC-A" + randomDigits(4); break;
    ...
    if (textInput) textInput.value = finalCode;
    createQR(finalCode, true);
}

// =====================================================
// CREATE QR CODE
// =====================================================
function createQR(value, saveHistory = true) {
    if (!value) return;
    ...
    if (typeof QRCode === "undefined") {
        if (qrContainer) {
            qrContainer.innerHTML = `
                <div style="color:#ff4500; font-size:13px; padding:20px; text-align:center;">
                    <strong>QR Library Not Loaded</strong><br><br>
    ...
    try {
        ...
        if (qrContainer) {
            qrContainer.innerHTML = `
                <div style="color:#ff4500; padding:20px; font-size:13px;">
                    QR generation failed.<br>Check browser console.
                </div>
            `;
        }
        return;
    ...
    if (!value.toLowerCase().includes("boss") && 
        !value.toLowerCase().includes("alauddin") && 
        !value.toLowerCase().includes("sunshine")) {
        playGenSound();
    }

    if (saveHistory) addToHistory(value);
}

function makeQR(text, saveToHistory = false) {
    createQR(text, saveToHistory);
}

// =====================================================
// MANUAL TEXT QR ENTER KEY
// =====================================================
if (textInput) {
...
function formatSmartInput(val) {
    ...
    const part3 = clean.substring(3, 4);
    const part4 = clean.substring(4);
...
if (textInput) {
    textInput.addEventListener("input", function () {
        ...
        const raw = currentVal.replace(/-/g, "");

        if (raw.length >= 2 && /^[A-Za-z][0-9]/.test(raw)) {
            const formatted = formatSmartInput(raw);
        ...
        if (textInput.value.trim() !== "") {
            createQR(textInput.value, false);
        } else {
            if (qrContainer) {
                qrContainer.innerHTML = `
                    <span style="color:#555; font-size:12px; font-style:italic;">
...
function downloadQR() {
    ...
    const fileName = (currentQRText || "BB_QR").replace(/[^a-z0-9]/gi, "_").substring(0, 50);
...
function addToHistory(value) {
    ...
    history = [value, ...history.filter(function (item) {
        return item !== value;
    })].slice(0, 10);

    try {
        localStorage.setItem("bbQrHistory", JSON.stringify(history));
    } catch (error) {
        console.log("Could not save history:", error);
    }
    renderHistory();
}
...
// QR TYPE BUTTONS
// =====================================================
document.querySelectorAll(".type-btn[data-type]").forEach(function (button) {
...
function animateFire() {
    if (!isFireActive || !fireCanvas || !fireCtx) return;
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        if (particles[i].life <= 0 || particles[i].y > fireCanvas.height + 20) {
...
// =====================================================
// EASTER EGG / BOSS MODE & LOVE MODE
// =====================================================
function checkEasterEgg(value) {
    ...
    if (lower.includes("alauddin") || lower.includes("boss")) {
        ...
        if (!body.classList.contains("royal-mode")) {
            ...
            if (mainCard) {
                setTimeout(function () {
                    mainCard.classList.remove("shake-card");
                }, 1000);
            }
        }
    } 
    // --- LOVE / SUNSHINE MODE ---
    else if (lower.includes("sunshine")) {
        ...
        if (!body.classList.contains("love-mode")) {
            body.classList.add("love-mode");

            if (mainCard) {
                mainCard.classList.add("heartbeat-card");
            }
            ...
            const loveFlash = document.createElement("div");
            loveFlash.innerHTML = "LOVE";
            loveFlash.className = "love-flash-text";
            document.body.appendChild(loveFlash);
            ...
            if (mainCard) {
                setTimeout(function () {
                    mainCard.classList.remove("heartbeat-card");
                }, 4500);
            }
        }
    }
...
// =====================================================
console.log("=================================");
console.log("BB QR PRO LOADED");
console.log("Login system: READY");
console.log("Old QR formats: RESTORED");
console.log("QR generator: READY");
console.log("Audio system: READY");
console.log("Royal & Love Modes: READY");
console.log("Fire system: READY");
console.log("Video error system: READY");
console.log("=================================");